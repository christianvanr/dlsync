...
class ParameterInjectorTest {
    ...
    void injectParametersDontCapture() {
        ...
        SchemaScript script = ScriptFactory.getDeclarativeScript("", "", ScriptObjectType.VIEWS, "VIEW1", content);
    ...
    void parameterizeObjectNameTest() {
        String content = "create or replace view as select * from TEST_DB.test_schema_1.table1 where tenant = 'test_tenant_id'";
        SchemaScript script = ScriptFactory.getDeclarativeScript("TEST_DB", "test_schema_1", ScriptObjectType.VIEWS, "VIEW1", content);
